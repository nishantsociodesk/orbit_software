...
import { createStorefrontCheckout } from '@/lib/storefrontApi';
...
export default function Checkout() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const tax = (subtotal - (subtotal * discount)) * 0.18;
      const shipping = 0;
      const total = (subtotal - (subtotal * discount)) + tax + shipping;
      const order = await createStorefrontCheckout({
      ...
      <div className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg mb-2" style={{ color: 'var(--text-muted)' }}>
            Thanks for shopping with Upfront Electronics.
          </p>
          <p className="text-sm mb-8" style={{ color: 'var(--text-muted)' }}>
            Order number: <span className="font-semibold" style={{ color: 'var(--text)' }}>{orderNumber}</span>
          </p>
          <Link
            href="/"
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {submitError && (
                <div className="rounded-lg border px-4 py-3 text-sm" style={{ borderColor: '#fecaca', color: '#b91c1c', backgroundColor: '#fef2f2' }}>
                  {submitError}
                </div>
              )}
            </div>
