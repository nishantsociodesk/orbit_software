...
export default function ProductGrid() {
  ...
  const categories = ['Electronics', 'Audio', 'Computers', 'Mobile', 'Wearables', 'Accessories'];
  const brands = ['Upfront', 'Orbit', 'Nova', 'Pulse'];
  const prices = ['₹0 - ₹10,000', '₹10,000 - ₹25,000', '₹25,000 - ₹50,000', '₹50,000+'];
  ...
  const displayedProducts = useMemo(() => {
    ...
    if (activeFilters.price.length > 0) {
      filtered = filtered.filter(p => {
        ...
        return activeFilters.price.some(range => {
          if (range === '₹0 - ₹10,000') return priceInINR <= 10000;
          if (range === '₹10,000 - ₹25,000') return priceInINR > 10000 && priceInINR <= 25000;
          if (range === '₹25,000 - ₹50,000') return priceInINR > 25000 && priceInINR <= 50000;
          if (range === '₹50,000+') return priceInINR > 50000;
          return false;
        });
      });
    }
    ...
    return filtered;
  }, [activeFilters, searchQuery, sortBy, products]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lg max-w-2xl mx-auto mb-6" style={{ color: 'var(--text-muted)' }}>
            Discover our curated electronics lineup, tuned for everyday performance
          </p>
