...
export default function MegaMenu({ category, isOpen, onMouseEnter, onMouseLeave }: MegaMenuProps) {
    ...
    return (
        <div
            className={`absolute top-full left-0 w-full bg-white border-t border-zinc-100 transition-all duration-300 transform origin-top z-40 ${isOpen ? 'opacity-100 visible translate-y-0 shadow-2xl' : 'opacity-0 invisible -translate-y-2'
                }`}
                ...
                <div className="grid grid-cols-5 gap-12">
                    {categories.map((section, idx) => (
                        <div key={idx} className="flex flex-col space-y-6">
                            <h3 className="font-heading font-medium text-lg text-black uppercase tracking-tight border-b border-black pb-2">
                                {section.title}
                            </h3>
                                ...
                                {section.items.map((item, itemIdx) => (
                                    <li key={itemIdx}>
                                        <Link
                                            ...
                                            className="text-sm font-light text-zinc-500 hover:text-black hover:translate-x-1 transition-all duration-200 block"
                                        >
                                            {item}
                                        </Link>
                                    </li>
                    ...
                    <div className="hidden lg:block relative h-full min-h-[200px] bg-zinc-100 p-6 flex flex-col justify-end">
                        <p className="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-2">New Arrivals</p>
                        <p className="font-heading text-2xl uppercase leading-none mb-4">Summer Collection 2026</p>
                        <Link href="/products" className="text-xs font-bold uppercase tracking-widest border-b border-black self-start pb-1">Shop Now</Link>
                    </div>
                </div>
            </div>
        </div>
