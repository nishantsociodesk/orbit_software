...
export function QuantitySelector({ quantity, onIncrement, onDecrement, className }: QuantitySelectorProps) {
    return (
        <div className={cn("inline-flex items-center border border-border bg-muted/50 rounded-none overflow-hidden", className)}>
            <Button
                variant="ghost"
                size="icon"
                onClick={onDecrement}
                className="h-10 w-10 hover:bg-muted rounded-none border-r border-border text-foreground"
            >
            ...
            <Button
                ...
                size="icon"
                onClick={onIncrement}
                className="h-10 w-10 hover:bg-muted rounded-none border-l border-border text-foreground"
            >
            ...
            <Table>
                <TableHeader className="bg-muted">
                    <TableRow className="hover:bg-transparent border-0">
                        <TableHead className="text-foreground font-black uppercase tracking-widest h-12">Nutrient</TableHead>
                        <TableHead className="text-foreground font-black uppercase tracking-widest h-12 text-right">Per Serving</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {items.map((item) => (
                        <TableRow key={item.label} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
